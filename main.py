    ...
    if config_path:
        ...
        for config in config_dict["models"]:
            config_model = {
                ...
                **({"use_openai_format": config["use_openai_format"]} if config.get(
                    "use_openai_format") is not None else {}),
...
def get_chat_synchronous(runpod, chat, model_name="gpt-3.5-turbo"):
    ...
    if response['status'] != "CANCELLED":
        ...
        elif isinstance(output_chunks, dict):
            # Handle case where output is a single dict instead of list
            choices = output_chunks.get("choices", [])
            if choices:
                # Check if it's a complete message
                message = choices[0].get("message", {})
                if message and "content" in message:
                    all_tokens = [message["content"]]
                else:
                    tokens = choices[0].get("tokens", [])
                    all_tokens.extend(tokens)

            # Get usage info
            usage = output_chunks.get("usage", {})
            if "input" in usage:
        ...
        # If we still have incomplete response, try to get the full content
        if not full_text or "<think>" in full_text:
            # Try to extract from different response structure
            if "output" in response and isinstance(response["output"], dict):
                choices = response["output"].get("choices", [])
                if choices and "message" in choices[0]:
                    full_text = choices[0]["message"].get("content", full_text)
